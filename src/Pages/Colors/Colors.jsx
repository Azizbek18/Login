...
function Colors() {
  ...
  const fetchUser = async (data)=>{
    try{
      const res = await axios.get('https://testaoron.limsa.uz/api/colors',data)
    ...
    <table className="min-w-full divide-y divide-gray-200">
      ...
      <tbody className="divide-y divide-gray-100 text-center">
        ...
        <tr key={item.id} className="hover:bg-gray-50 transition duration-200">
          ...
          <td className="border-2 border-[#c8c8c8] px-6 py-4">{item.color_en}</td>
          <td className="border-2 border-[#c8c8c8] px-6 py-4">{item.color_ru}</td>
          <td className="border-2 border-[#c8c8c8] px-6 py-4">{item.color_de}</td>
